...
const adoptadogblog= document.getElementById('blogsec');

blogdiv.forEach(function(character) {
  const newCharacter = document.createElement('div');
  blogdiv.setAttribute('class', 'blogdiv');


  const image = document.createElement('img');
  image.setAttribute('src', ./images/blog-3.jpg);
  blogdiv.appendChild(image);

  const headline = document.createElement('h2');
  headline.innerText = bdiv.header;
  blogdiv.appendChild(headline);

  const job = document.createElement('h2');
  text.innerText = bdiv.blog;
  blogdiv.appendChild(job);



el.appendChild(adoptadogblog);
});
